    ...
    try {
        ...
        try {
            ...
            if (existingData.ids.length > 0) {
                console.log(
                    `   ⏭️  Collection already exists in cloud with ${existingData.ids.length} items - skipping`
                );
                return;
            }
        } catch {
        ...
        const metadata =
            ...
            Object.keys(localCollection.metadata).length > 0
                ? localCollection.metadata
                : { migrated: "true" };
        ...
        for (let i = 0; i < totalItems; i += BATCH_SIZE) {
            ...
            if (batchDocuments && batchDocuments.length > 0) {
                addParams.documents = batchDocuments.filter(
...
async function main() {
    ...
    console.log(
        `  - Local URL: ${process.env.CHROMA_URL || "http://localhost:8000"}`
    );
    ...
    try {
        ...
        console.log(
            `   ✅ Local connection OK (${localCollections.length} collections)`
        );

        const cloudCollections = await cloudClient.listCollections();
        console.log(
            `   ✅ Cloud connection OK (${cloudCollections.length} collections)`
        );
        ...
        console.log("\n📋 Next steps:");
        console.log(
            "   1. Update your .env file with CHROMA_API_KEY, CHROMA_TENANT, CHROMA_DATABASE"
        );
